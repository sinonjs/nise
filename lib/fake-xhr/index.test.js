...
describe("FakeXMLHttpRequest", function () {
        ...
        if (supportsArrayBuffer) {
            describe("with ArrayBuffer support", function () {
                ...
                it("returns utf-8 strings correctly when responseType='arraybuffer'", function () {
                    ...
                    var expectedString = toBinaryString(new TextEncoder("utf-8").encode("\xFF").buffer);

                    assertArrayBufferMatches(this.xhr.response, expectedString);
                });
                ...
                it("returns binary data correctly when responseType='arraybuffer'", function () {
                    ...
                    this.xhr.respond(200, { "Content-Type": "application/octet-stream" }, toBinaryString(buffer));

                    assertArrayBufferMatches(this.xhr.response, toBinaryString(buffer));
                });
            });
        }
    ...
    describe("stub XHR", function () {
        ...
        it("calling restore removes temporary method", function () {
            ...
            refute.defined(fakeXhr.restore);
        });
